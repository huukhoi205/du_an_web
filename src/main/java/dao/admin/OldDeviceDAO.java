	...
	public class OldDeviceDAO {
	    ...
	    public boolean insert(OldDevice o) {
	        String sql = "INSERT INTO thumuacu (MaND,TenMay,HangSX,TinhTrang,GiaDeXuat,GiaThoaThuan,TrangThai,DiaChiNhan) VALUES (?,?,?,?,?,?,?,?)";
	        try (Connection conn = DatabaseConnection.getConnection();
	             PreparedStatement ps = conn.prepareStatement(sql)) {
	            ...
	            ps.setString(8, o.getDiaChiNhan());
	    ...
	    public boolean update(OldDevice o) {
	        String sql = "UPDATE thumuacu SET MaND=?, TenMay=?, HangSX=?, TinhTrang=?, GiaDeXuat=?, GiaThoaThuan=?, TrangThai=?, DiaChiNhan=? WHERE MaTMC=?";
	        try (Connection conn = DatabaseConnection.getConnection();
	             PreparedStatement ps = conn.prepareStatement(sql)) {
	            ps.setInt(1, o.getMaND());
	            ps.setString(2, o.getTenMay());
